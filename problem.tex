...
This observer formulation differs from that provided in equation \ref{eq:observerfunction} in that $\tilde{r}$ and $\hat{r}$ are provided as separate inputs. Though a true Luenberger observer is driven by the output difference $\tilde{r} - \hat{r}$, such a scheme is not possible due to the way the problem has been simplified. 
...
Successful IF converges in local neighbourhood - since not invariant, don't expect global convergence.
TO IMPLEMENT - WILL DESIGN SIMULATION + DO EXPERIMENTAL VALIDATION
