...
\subsection{Rigid Body Kinematics}
	...
	\subsubsection{Scanning Laser Rangefinder Dynamic Model}
		...
		Unit vector defined in the body fixed frame  - ${^{A}\mathbf{s}(t)} \in T\mathbb{R}^3$.\\+\\
		Range $r(t) \in \mathbb{R}^{0+}$, defining range from $^{F}_{F}\mathbf{p}^{}_{A}(t)$ to nearest object in environment in direction ${^{F}\mathbf{n}(t)} = {^{F}_{F}\mathbf{R}^{}_{A}(t)}\:{^{A}\mathbf{s}(t)}$
