...
\section{Rigid Body Kinematics}
	...
	\subsection{Scanning Laser Rangefinder Dynamic Model}
		A scanning laser rangefinder fixed to a moving rigid body. Pose defined as moving rigid body defined above (S,T,W). State comprised of pose and scanning direction - Unit vector defined in the body fixed frame  - ${^{A}\mathbf{d}(t)} \in S^2$.
		
		Provides measurement of range $r(t) \in \mathbb{R}^{0+}$, defining range from $^{F}_{F}\mathbf{p}^{}_{A}(t)$ to nearest object in environment in direction ${^{F}\mathbf{d}(t)} = {^{F}_{F}\mathbf{R}^{}_{A}(t)}\:{^{A}\mathbf{d}(t)}$
		
		Scan direction in sensor frame depends on model of sensor, for Hokuyo UBG 04 LX-FO1 used in this research, model is shown in section \ref{sec:scanmodel}.
