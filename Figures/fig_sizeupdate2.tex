...
\begin{figure}
	...
	\begin{tikzpicture} [scale = 3,tdplot_main_coords] %fix main coords!!!
		...
		\node [black] at (-0.3,6.5*\measscale,0.6) {$\bar{\mathbf{p}}_{measured}$};
		\node [blue] at (-0.25,6.5*\predscale,0.5) {$\bar{\mathbf{p}}_{predicted}$};
		%\node [green] at (-0.45,7,0.55) {$\bar{\mathbf{p}}_{update}$};	
		
		%labels
		\node [red] at (-0.4,9.25,0.05) {${^{A}\mathbf{n}(ii)}$};
		\node [red] at (-1.1,9.25,0.05) {${^{A}\mathbf{n}(ii-1)}$};
		\node [red] at (-0.2,9.25,0.4) {${^{A}\mathbf{n}(ii-n_{steps})}$};	
		\node [red] at (-1.2,9.25,0.5) {${^{A}\mathbf{n}(ii-1-n_{steps})}$};
