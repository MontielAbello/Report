\begin{figure}
	...
	\begin{tikzpicture} [scale = 3,tdplot_main_coords] %fix main coords!!!
		...
		\node [blue,anchor=south east] at (-0.55,6.5*\measscale,0.65) {$\tilde{\mu}_r$};
		\node [red,anchor=north east] at (-0.42,6.5*\predscale,0.6) {$\hat{\mu}_r$};
		\node [green] at (-0.4,6.3,0.52) {$s_{update}$};	
	...
	\end{tikzpicture}
	  \caption{Size update - case 2: difference in mean ranges used to determine size update.}
	  \label{fig:size2}
\end{figure}
