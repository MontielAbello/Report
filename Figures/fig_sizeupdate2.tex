...
\begin{figure}
	...
	\begin{tikzpicture} [scale = 3,tdplot_main_coords] %fix main coords!!!
		...
		%labels
		\node [red] at (-0.4,9.25,0.05) {${^{F}\mathbf{d}(ii)}$};
		\node [red] at (-1.1,9.25,0.05) {${^{F}\mathbf{d}(ii-1)}$};
		\node [red] at (-0.2,9.25,0.4) {${^{F}\mathbf{d}(ii-n_{scans})}$};	
		\node [red] at (-1.2,9.25,0.5) {${^{F}\mathbf{d}(ii-1-n_{scans})}$};
