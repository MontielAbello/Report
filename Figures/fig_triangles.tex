\begin{figure}
	\begin{tikzpicture} [scale = 2.5,tdplot_main_coords] %fix main coords!!!

		%Frame {B}
		\draw [thick,->](0, 0, 0)--(-2, 0, 0) node[anchor=east]{$y$};
		\draw [thick,->](0, 0, 0)--(0, 2, 0) node[anchor=east]{$x$};
		\draw [thick,->](0, 0, 0)--(0, 0, 2) node[anchor=south]{$z$};
		\node at (0.1,-0.1,0) {\{B\}};
		%points
		%REDO ALL POINTS AND TRIANGLES TO MATCH AXES
		
		\node[anchor=west] at ( 1,-1,-1) {$1$};
		\node[anchor=west] at ( 1,-1, 1) {$2$};
		\node[anchor=north] at (-1,-1,-1) {$3$};
		\node[anchor=north west] at (-1,-1, 1) {$4$};
		\node[anchor=north] at ( 1, 1,-1) {$5$};
		\node[anchor=south] at ( 1, 1, 1) {$6$};
		\node[anchor=east] at (-1, 1,-1) {$7$};
		\node[anchor=south] at (-1, 1, 1) {$8$};
				
		%cube lines
		\draw (-1,-1,-1)--(-1,-1, 1);   %1-2
		\draw (-1,-1,-1)--(-1, 1,-1);   %1-3
		\draw (-1,-1,-1)--( 1,-1,-1);   %1-5
		\draw (-1,-1, 1)--(-1, 1, 1);   %2-4
		\draw (-1,-1, 1)--( 1,-1, 1);   %2-6	
		\draw (-1, 1,-1)--(-1, 1, 1);   %3-4 	 
		\draw (-1, 1,-1)--( 1, 1,-1);   %3-7	
		\draw (-1, 1, 1)--( 1, 1, 1);   %4-8 
		\draw ( 1,-1,-1)--( 1,-1, 1);   %5-6	
		\draw ( 1,-1,-1)--( 1, 1,-1);   %5-7 
		\draw ( 1,-1, 1)--( 1, 1, 1);   %6-8 
		\draw ( 1, 1,-1)--( 1, 1, 1);   %7-8 
		%triangle lines
		\draw [dashed] ( 1,-1, 1)--(-1,-1,-1);   %2-3
		\draw [dashed] (-1,-1, 1)--(-1, 1,-1);   %4-7
		\draw [dashed] ( 1, 1, 1)--(-1, 1,-1);   %6-7 
		\draw [dashed] ( 1,-1, 1)--( 1, 1,-1);   %2-5
		\draw [dashed] ( 1,-1, 1)--(-1, 1, 1);   %2-8
		\draw [dashed] ( 1,-1,-1)--(-1, 1,-1);   %1-7
		
	\end{tikzpicture}
	  \caption{caption}
	  \label{fig:triangles}
\end{figure}



