...
\begin{figure}
	...
	\begin{tikzpicture} [scale = 3,tdplot_main_coords] %fix main coords!!!
		...
		\node [blue] at (-0.85,6.5*\predscale,1.05) {$\hat{\bm{\mu}}_\mathbf{p}$};
		\node [black] at (-0.35,6.5*\measscale,0.5) {$\tilde{\bm{\mu}}_\mathbf{p}$};
		\node [green] at (-0.5,7,0.55) {$\mathbf{p}_{update}$};		

		%labels
		\node [red] at (-0.4,9.25,0.1) {${^{F}\mathbf{d}(ii)}$};
		\node [red] at (-1.6,9.25,0.1) {${^{F}\mathbf{d}(ii-1)}$};
		\node [red] at (-0.2,9.25,1.3) {${^{F}\mathbf{d}(ii-n_{scans})}$};	
		\node [red] at (-1.3,9.25,1.3) {${^{F}\mathbf{d}(ii-1-n_{scans})}$};
